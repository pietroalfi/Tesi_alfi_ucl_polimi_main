...
def build_config(
    ticker="INTC",
    period_starts= ["20241001 120000"], 
    fundamental_file_path="/home/pietro/ucl-thesis-main/ucl-thesis-main/LOB_data_preprocess_merge/Merged_data/INTC_merged_mid_price_train.pkl",
    ...
    # 2) Carichiamo il file dei fondamentali e impostiamo lo start_time
    fundamental_df = pd.read_pickle(fundamental_file_path)
    fundamental_df.index = pd.to_datetime(fundamental_df.index)

    # Selezioniamo il primo timestamp nel file maggiore del candidato
    filtered = fundamental_df.loc[fundamental_df.index > start_time]
    if filtered.empty:
        raise ValueError("Nessun timestamp nel file maggiore del candidato scelto.")

    # Il nuovo start time sarà il primo timestamp valido
    start_timestamp = filtered.index.min()

    # Convertiamo in nanosecondi
    start_time = start_timestamp.value
    print("Period of start:", pd.Timestamp(start_time))
    # 3) Impostiamo mkt_open :
    mkt_open = start_time
    ...
    agents.extend(
        [
            ValueAgent(
                ...
                sigma_n=sigma_n,
                r_bar=r_bar,
                kappa=val_kappa,  # riga dopo ->  sigma_s = 100,
