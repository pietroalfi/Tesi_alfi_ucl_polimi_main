...
def build_config(
    ...
    ########################################################################################################################
    ############################################### AGENTS CONFIG ##########################################################
    # Randomly gen historical weekday to simulate.
    if random_date == True:
        range_start = datetime.strptime(historical_date_start, '%Y%m%d')
        range_end = datetime.strptime(historical_date_end, '%Y%m%d')
        historical_date_start = random_weekday(range_start, range_end)
        historical_date_end = historical_date_start

    # Convert to nanoseconds.    
    historical_date_start = datetime_str_to_ns(historical_date_start)
    historical_date_end = datetime_str_to_ns(historical_date_end)

    mkt_open = historical_date_start + str_to_ns(start_time)
    mkt_close = historical_date_end + str_to_ns(end_time)
    ...
    kernelStartTime = int(pd.to_datetime(historical_date_start).to_datetime64())
