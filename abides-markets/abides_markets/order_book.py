...
class OrderBook:
    ...
    def execute_order(self, order: Order) -> Optional[Order]:
        ...
        old_best_ask = self.asks[0].price if self.asks else None
        old_best_bid = self.bids[0].price if self.bids else None
        ...
        if len(book) == 0:
            ...
            if order.side.is_bid():
                diff_price = matched_order.limit_price - old_best_ask 
            else:
                diff_price = old_best_bid - matched_order.limit_price
    ...
    def get_transacted_volume(self, lookback_period: str = "10min") -> Tuple[int, int, List[Tuple[int, int]], List[Tuple[int, int]]]:
        """Method retrieves the total transacted volume for a symbol over a lookback
        period finishing at the current simulation time.
    
        Aggregates transacted volume per price level for bid and ask orders.
        ...
        # Original calculation using self.buy_transactions and self.sell_transactions
        ...
        # Aggregation from history for per price level volumes
        bid_volume_per_level = {}
        ask_volume_per_level = {}

        for record in self.history:
            if record.get("type") != "EXEC":
                continue
            if record.get("time") < window_start:
                continue
            exec_price = record.get("price") if record.get("price") is not None else self.last_trade
            if record.get("side") == "BUY":
                bid_volume_per_level[exec_price] = bid_volume_per_level.get(exec_price, 0) + record.get("quantity", 0)
            elif record.get("side") == "SELL":
                ask_volume_per_level[exec_price] = ask_volume_per_level.get(exec_price, 0) + record.get("quantity", 0)

        bid_volume_per_level_list = sorted(bid_volume_per_level.items(), key=lambda x: x[0], reverse=True)
        ask_volume_per_level_list = sorted(ask_volume_per_level.items(), key=lambda x: x[0])

        return (buy_transacted_volume, sell_transacted_volume, bid_volume_per_level_list, ask_volume_per_level_list)
