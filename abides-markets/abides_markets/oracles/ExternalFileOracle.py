...
class ExternalFileOracle:
    ...
    def load_fundamentals(self):
        ...
        """
        fundamentals = dict()
        logger.debug("Oracle: loading fundamental price series...")
        for symbol, params_dict in self.symbols.items():
            fundamental_file_path = params_dict['fundamental_file_path']
            logger.debug("Oracle: loading {}", fundamental_file_path)
            fundamental_df = pd.read_pickle(fundamental_file_path)
            fundamentals.update({symbol: fundamental_df})

        logger.debug("Oracle: loading fundamental price series complete!")
        return fundamentals

    def get_daily_open_price(self, symbol, mkt_open):

        # Remember market open time.
        self.mkt_open = mkt_open

        logger.debug("Oracle: client requested {} at market open: {}", symbol, mkt_open)
    ...
    def getPriceAtTime(self, symbol, query_time):
        ...
        logger.debug("Oracle: client requested {} as of {}", symbol, query_time)
        ...
        if time_of_query < series_open_time:  # time queried before open
            ...
            #print("LOWER: ", lower_idx, lower_val," UPPER: ", upper_idx, upper_val)

            #return fundamental_series.asof(time_of_query)
            #print(time_of_query, fundamental_series.asof(time_of_query))

            logger.debug(f"DEBUG: lower_idx: {lower_idx}, lower_val: {lower_val}, upper_idx: {upper_idx}, upper_val: {upper_val}")
            ...
            logger.debug("Oracle: latest historical trade was {} at {}. Next historical trade is {}. "
    ...
    def observe_price(self, symbol, currentTime, sigma_n=0, random_state=None):
        ...
        #print(pd.Timestamp(currentTime), true_price)
    ...
    def getInterpolatedPrice(self, current_time, time_low, time_high, price_low, price_high):
        ...
        """
        logger.debug(f'DEBUG: current_time: {current_time} time_low {time_low} time_high: {time_high} price_low:  {price_low} price_high: {price_high}' )
        delta_y = price_high - price_low
        delta_x = (time_high - time_low).total_seconds()

        slope = delta_y / delta_x if price_low != price_high else 0
        x_fwd = (pd.Timestamp(current_time) - time_low).total_seconds()
