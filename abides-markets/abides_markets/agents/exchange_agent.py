...
class ExchangeAgent(FinancialAgent):
    ...
    def kernel_terminating(self) -> None:
        ...
        bid_volume, ask_volume, _, _ = self.order_books["ABM"].get_transacted_volume(
        ...
        for symbol in self.symbols:
            bid_volume, ask_volume, _, _ = self.order_books[symbol].get_transacted_volume(
                self.current_time - self.mkt_open
            )
    ...
    def handle_frequency_based_data_subscription(
        ...
        if isinstance(data_sub, self.L1DataSubscription):
            ...
            bid_volume, ask_volume, bid_volume_per_level, ask_volume_per_level = book.get_transacted_volume(data_sub.lookback)
            messages.append(
                TransactedVolDataMsg(
                    symbol,
                    book.last_trade,
