...
import numpy as np
import argparse  # <-- added to parse CLI flags

# Parse CLI argument --resume (True if resuming from checkpoint)
...
tune.run(
    ...
    resume=args.resume,  # Resume training if --resume is passed
    ...
    config={
        ...
        "min_iter_time_s": 1, 
        "callbacks": MyCallbacks,  # Custom metrics + replay buffer save/restore
        # Performance
