...
class PartialMetricsStore:
    ...
    data = {"reward": [], "spread": [], "holdings": [], "value": [], "action": [], "mid_price": [], "current_time": [],"total_excuted_volume": [] }
    file_initialized = False  # Controlla se il file è già stato inizializzato

    @classmethod
    def reset(cls):
        print("Reset delle metriche parziali")
        cls.data = {"reward": [], "spread": [], "holdings": [], "value": [], "action": [], "mid_price": [], "current_time": [] }

    @classmethod
    def update(cls, reward, spread, holdings, value, action, mid_price, current_time):
        cls.data["reward"].append(reward)
        cls.data["spread"].append(spread)
        cls.data["holdings"].append(holdings)
        cls.data["value"].append(value)
        cls.data["action"].append(action)
    ...
    def get_averages(cls, output_file='training_data.csv'):
        ...
        actions = np.zeros(4)  # Supponendo ci siano 4 possibili azioni
        for val in cls.data["action"]:
            actions[val] += 1
...
class MyBaseCallbacks_HF(DefaultCallbacks):
    """
    ...
    def on_episode_step(
        ...
        **kwargs,
        ...
        PartialMetricsStore.update(reward, spread, holdings, value, action, mid_price, current_time)
    ...
    def on_episode_end(
        ...
        **kwargs,
        ...
        episode.custom_metrics["Daily Net Cash Performance"] = episode.user_data["value"][-1] - 200_000
