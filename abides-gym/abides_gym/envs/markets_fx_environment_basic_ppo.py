...
class SubGymFxEnv_basic_ppo(AbidesGymMarketsEnv):
    """
    ...
    def __init__(
        ...
        self.action: tuple[float, float] = (0.0, 0.0)
    ...
    def _map_action_space_to_ABIDES_SIMULATOR_SPACE(self, action: np.ndarray
            ) -> List[Dict[str, Any]]:
        """
        Interpretiamo action = [d_b, d_a] come offset continuo da
        aggiungere a best_bid e best_ask.

        Esempio: 
           limit_price_buy = best_bid + round(d_b)
           limit_price_sell = best_ask + round(d_a)
        """
        db_float, da_float = action
        
        # Calcolo i prezzi limite finali
        limit_buy = int(round(self.mid_price - db_float))
        limit_sell = int(round(self.mid_price + da_float))
        self.penalty = 0 if limit_buy < limit_sell else 1e3
        print("Bid price:", limit_buy)
        print("Ask price:", limit_sell)
        # Esempio: se l'agente ha troppa posizione LONG, evito buy, etc.
        holdings = self.state[2, 0] if self.state is not None else 0

        actions_list = []
        # In ogni caso, prima "cancello tutti" se voglio rimuovere ordini vecchi.
        actions_list.append({"type": "CCL_ALL"})

        # Se holdings < max_holdings, allora un LMT BUY
        ...
        # Se holdings > -max_holdings, allora un LMT SELL
        if holdings > -self.max_holdings:
            actions_list.append({
                "type": "LMT",
                "direction": "SELL",
        ...
        # Salvo per debug
        self.action = (db_float, da_float)

        # Ritorno la lista di azioni da passare ad ABIDES
        return actions_list
    ...
    def raw_state_to_state(self, raw_state: Dict[str, Any]) -> np.ndarray:
        ...
        self.custom_metrics_tracker.fundamental_value = self.kernel.oracle.observe_price(symbol="EURUSD", currentTime=current_time)
    ...
    def raw_state_to_reward(self, raw_state: Dict[str, Any]) -> float:
        ...
        spread_trip = sum(order.fill_price * (1 if order.side == "ASK" else -1) 
                  for order in inter_wakeup_executed_orders) if round_trip else 0
        self.m2m = portfolio_mkt_value_t

        PnL_reward = (portfolio_mkt_value_t - self.portfolio_mkt_value_tm1) * self.kappa+ 100*round_trip*spread_trip
        ...
        total_reward = self.w_direc*directional_reward + (1 - self.w_direc)*(Execution_PnL) -self.penalty 
        print("Eps:", self.eps)
        print("Mask part:",  -abs(total_reward) * self.eps)
        total_reward -=  self.eps*20
        ...
        print("Pnl_reward:",(portfolio_mkt_value_t - self.portfolio_mkt_value_tm1))
        print("Directional reward:",directional_reward)
        print("round trip:", round_trip)
        print("Signal part:", self.w_direc*directional_reward)
        print("Rest:", (1 - self.w_direc)*(PnL_reward) )
    ...
    def raw_state_to_update_reward(self, raw_state: Dict[str, Any]) -> float:
        ...
        return (self.m2m-self.starting_cash)
