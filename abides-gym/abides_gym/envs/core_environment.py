...
class AbidesGymCoreEnv(gym.Env, ABC):
    ...
    class CustomMetricsTracker(ABC):
        ...
        # These variables were typed to floats in prior configuration.
        cash_balance: int = 0
        holdings: int = 0
        bid: int = 0
        ask: int = 0
        ...
        bid_volume: int = 0
        ask_volume: int = 0
        bid_volume_agent: int = 0
        ask_volume_agent: int = 0
        bid_exe_vol_agent: int = 0 
        ask_exe_vol_agent: int = 0        
        avg_future_price: float = 0
        fundamental_value: float = 0
        current_time: float = 0
        directional_signal_0: float = 0
        directional_signal_1: float = 0
        directional_signal_2: float = 0
        portfolio_value: float = 0
        pnl_reward: float = 0
        execution_rewrad: float = 0
        directional_reward: float = 0
    ...
    def reset(self):
        ...
        num_max_steps_per_episode = 18_000 # need to better understand what this var does.
        ...
        self.mkt_close = background_config_state["mkt_close"]
        print("mkt_close: ", pd.Timestamp(self.mkt_close))
        ...
        gym_agent = self.gymAgentConstructor(
            nextid,
            background_config_state["ticker"],
        ...
        self.kernelStartTime = config_state["start_time"]
        ...
        # Dynamic parameters which need to reset (ray).
        self.step_index: int = -1
        self.farthest_executed_idx: int = 0
        hist_keys = ["bid_hist", "ask_hist", "bid_volume_hist", 
                     "ask_volume_hist", "bid_volume_agent_hist", "ask_volume_agent_hist"]
        self.hist_arrays = {key: np.zeros(shape=(self.state_timesteps_back,)) for key in hist_keys}
        self.volume_hist = np.zeros(shape=(50,))
        self.price_hist_5 = np.zeros(shape=(5,))
        self.price_hist_30 = np.zeros(shape=(30,))
