...
class MyCallbacks(DefaultCallbacks):
    ...
    def on_episode_end(self, *, worker: RolloutWorker, base_env: BaseEnv,
        ...
        # Data for Enviroment Metrics (only in evaluation)
        if episode.custom_metrics.get("evaluation", False):  # oppure un flag che usi per distinguere
        save_path = "evaluation_data.csv"
        with open(save_path, "a") as f:
            writer = csv.writer(f)
            if os.stat(save_path).st_size == 0:
                writer.writerow(["episode_id", "timestep", "timestamp", "fundamental_price", "mid_price"])
            for t, (time, fund, mid) in enumerate(zip(
                episode.user_data["current_time"],
                episode.user_data["fundamental_value"],
                episode.user_data["mid"]
            )):
                writer.writerow([episode.episode_id, t, time, fund, mid])
