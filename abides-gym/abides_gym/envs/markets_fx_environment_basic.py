...
class SubGymFxEnv_basic(AbidesGymMarketsEnv):
    """
    ...
    def _map_action_space_to_ABIDES_SIMULATOR_SPACE(
        ...
        buy_actions, sell_actions = [0,1], [0,2]
        """
        if holdings >= self.max_holdings and action in buy_actions:
            return []

        elif holdings <= -self.max_holdings and action in sell_actions:
            return [] 
        """
        self.eps = 0.0
        if action == 0:
    ...
    def raw_state_to_info(self, raw_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.debug_mode == True:
            return {
                ...
                "bids": bids,
                "asks": asks,
                "bid sparsity" : bid_sparsity,
                "ask sparsity" : ask_sparsity,
                "bid volume profile" : bid_volume_profile,
                "ask volume profile" : ask_volume_profile,
                "total excuted volume" : tot_executed_volume,
                "bid executed volume" : transacted_volume_at_levels_bid,
                "ask executed volume" : transacted_volume_at_levels_ask
